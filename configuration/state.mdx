...
type State = {
  ...
  automatedAction?: AutomatedActionRequest;
};
```
...
`automatedAction` is an engine-reserved field. When an effect sets it, Trace API performs the
described action in the background once the current link is saved, and grants that action to the
target group in `nextActions` automatically.
...
It is set on the **state root**, not on `state.data`. See
[Automated actions](/configuration/action/automated-actions) for the full payload and its
constraints.
