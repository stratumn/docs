...
title: 'Notifications'
icon: 'bell'
description: 'Notification sent as emails'
---

Notifications are a way to communicate updates, alerts, or information to users or groups.
The notifications, usually in email format, are sent by Notification-API.

## Overview

Notifications in this system serve multiple purposes, including:

- Alerting users about changes in workflows.
- Sending important updates or reminders via email.
- Inviting new users to join Stratumn...

## How it works in configuration

The notifications are not directly sent through configuration or dsl package. They are stored under `state.notifications` an an array. Only when doing an action at runtime, Trace API will look into this `newState.notifications` and loop on each object, then build the notification and call the correct template on Notification API.

The action needs to provide the correct data to the `state.notifications`, typed as `NotificationOptions` :
...
<Tabs>
  <Tab title="Documentation">
    ...
    <ParamField path="template?" type="Template">
      ...
      type Template = {
        type?: "HTML";
        version?: string; // By default, is 1.0.0
        name: TemplateName;
    ...
    `TemplateConfigMap` contains the core of your template. Please see [below](/configuration/action/notifications#email-templates) to check all available templates.
    </ParamField>
...
meta: Readonly<LinkMetaData<GroupLabel, PaymentProposalActionLabel>>,
groupLabel: string,
data: PaymentProposalData
): NotificationOptions {
...
async function markPaymentReceivedEffect(dsl: PaymentProposalContext<FormData>) {
...
The `schema-full` template has a rich HTML structure including metadata, workflow context, trace context, last and next actions from the trace and any additional information provided.
