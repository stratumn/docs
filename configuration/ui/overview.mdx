...
- **interpreter** _(Optional)_: Used for `Progress` and `Number` types to define how the values are interpreted.
...
These configurations do not store confidential information; they only handle display preferences. When users revisit the same URL (workflow overview with a specific ID), hey will retrieve exactly the same layout.

## Display Menu

The Display Menu available on the top right of the table enables to select the columns that users want to see.

<img
  src="/images/configuration/ui/overview/display-menu.png"
  style={{ borderRadius: '0.5rem', height: '400px' }}
/>

Selecting a column will add an object with its key at the end of the ‘columns’ field of the user display configuration document. Deselecting one will simply remove the object with the corresponding key from it.

Hence note that deselecting then reselecting a column will result in the column being pushed at the end of the table.

## Layout customization

The **Customize layout** mode lets the user reorder the columns by drag-n-dropping the headers and resize their width, as well as resize the rows height (only the first row is resizable and this will update all the rows height).

<Frame caption="Reordering columns">
  <img
...
Reordering the columns will result in changing the order elements in the columns field of the user display configuration document, while resizing their width will change the width field of the corresponding column. Resizing the first row will result in modifying the rowsHeight field at the root of this document.

<Frame caption="Resizing columns">
  <img
    src="/images/configuration/ui/overview/resizing.png"
    style={{ borderRadius: '0.5rem' }}
  />
</Frame>

> **Note**: to improve reactivity of the page when customizing the layout, as long as the mouse is held down, the drag-n-drop and resizing only dynamically affects the column headers. The table itself is re-rendered only once the mouse is released. (An intermediary feature could be a debounced update of the table, i.e. as long as the mouse has not moved for at least 250 ms for example).

# Exporting the table

It is possible to export the data by clicking on the Export icon of the table headers, as a csv file.
All the table display configuration currently setup will apply when exporting: sorting, filtering and columns selection. The headers of the csv columns will be the names of the table columns.
Each view configured in the columns defines its own export mechanism, mostly to replicate what is displayed in the table UI.

## Export Configuration

So far, a small amount of things are configurable for export, mainly at the root of the config, under “exportCsv”:

`filename`: the name of the exported .csv file. Actual file name will be: date-filename.csv.
_Example: 2021-07-20-tabel_export.csv_

`delimiter`: the delimiter string used to build the .csv file. Default: ‘;’.

```json
{
  "exportCsv": { "filename": "My Export", "delimiter": "," }
}
```

# Templating

Templating means pre-packaging a set of display configurations that users can toggle from the Display Menu. When a user selects a template from the Display Menu, his current local storage cached display configuration will be replaced by the template’s definition. Templates also have labels displayed in the menu to let the user know what they are about.
...
It corresponds to the following elements of the table global configuration:
...
The `default` label is reserved to the field `defaultDisplay` of the table configuration.
