...
migration**: a new file, a new `key`, a new entry in `actions`.
...
<Note>
  ...
  no traces, so leaving it in `actions` is free — and it is the record of what was done
  to the data.
</Note>
...
- [ ] it is a **new file with a new `key`** — no migration that has already run was edited
